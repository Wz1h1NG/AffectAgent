...
def parse_args():
    parser = argparse.ArgumentParser(description="Construct SFT data for the Query Planner")
    parser.add_argument(
        "--dataset",
        type=str,
        default="mer2023",
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="affectagent/artifacts/sft_data",
...
def call_gpt4o_for_q(client, model, sample_data, candidate_labels, lang="zh"):
    """
    Call the teacher model to generate the Query Planner's three cognitive queries.
    """
...
def main():
    ...
    print(f"Processing {len(samples)} samples to generate Query Planner SFT data...")
    ...
    output_path = os.path.join(args.output_dir, "query_planner_sft.jsonl")
    with open(output_path, "w", encoding="utf-8") as f:
        for item in sft_data:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")
            
    print(f"Query Planner SFT data saved to {output_path} ({len(sft_data)} samples)")
