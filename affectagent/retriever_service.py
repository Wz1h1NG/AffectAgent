...
class DualChannelRetriever:
    """
    Frozen AffectAgent dual-channel Retriever.
    该模块为冻结状态，不参与梯度更新。
    
    包含两个完全独立的通道：
    1. Cognitive semantic retrieval for the Evidence Filter and Generator.
    2. Perceptual audiovisual retrieval for RAAF.
    """
    ...
    def retrieve_channel_A(self, queries_dict, top_k=3, exclude_sample_id=None):
        """
        Cognitive channel: retrieve three independent pools from Query Planner queries.
        
        Args:
            queries_dict: three queries generated by the Query Planner. Example:
                {
                   "support": {"query_text": "..."},
        ...
        Returns:
            candidates: three groups passed to the Evidence Filter
                {
                   "support": [{"id": ..., "text": ..., "score": ...}, ...],
                   "confusion": [...],
                   "counter": [...]
                }
        """
        if self.semantic_faiss is None or self.encoder is None:
            raise RuntimeError(
                "Cognitive semantic index is not loaded. Pass semantic_index_dir to DualChannelRetriever."
            )
    ...
    def retrieve_channel_B(self, current_sample_features, top_k=1, exclude_sample_id=None):
        """
        Channel B: retrieve perceptually similar audiovisual evidence for RAAF.
        This channel is independent of the Query Planner and uses audiovisual similarity.
...
DoubleChannelRetriever = DualChannelRetriever
