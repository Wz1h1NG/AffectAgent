...
输出格式要求：
{
  "prediction": "angry",
  "confidence": 0.85,
  "reasoning": "待分析样本的音频特征经融合后与support证据的被动攻击模式一致。对比contrast证据的平和语气，待分析样本明显更紧张急促。counter证据描述的真诚认同与待分析样本不符。综合判断为angry。"
}"""
...
3. [Contrast Evidence]: Evidence used to distinguish easily confused emotions.
...
- "reasoning": A 2-4 sentence reasoning process. You must cite content from the [Support/Contrast/Counter] evidence and conduct a comparative analysis combined with the current sample's multimodal features.

Output format requirement:
{
  "prediction": "angry",
  "confidence": 0.85,
  "reasoning": "The audio features of the analyzed sample, after fusion, are consistent with the passive-aggressive mode of the support evidence. Compared to the peaceful tone of the contrast evidence, the analyzed sample is obviously more tense and rushed. The sincere agreement described in the counter evidence does not match the analyzed sample. Overall judgment is angry."
}"""

def parse_args():
    parser = argparse.ArgumentParser(description="Construct SFT data for Agent-G")
    parser.add_argument("--dataset", type=str, default="mer2023", help="Target dataset (mer2023, meld, iemocap)")
    parser.add_argument("--s-data-path", type=str, default="retrieval/mmoa_lite/artifacts/sft_data/agent_s_sft.jsonl")
    parser.add_argument("--index-dir", type=str, default="retrieval/mmoa_lite/artifacts/semantic_index")
    ...
    parser.add_argument("--output-dir", type=str, default="retrieval/mmoa_lite/artifacts/sft_data")
...
def call_gpt_for_g(client, model, sample_id, subtitle, ground_truth, multimodal_desc, ev_texts, fusion_partner_id, lang="zh"):
    """Call LLM to generate reasoning and format output for Agent-G"""
    
    evidence_prompt = ""
    for role, ev in ev_texts.items():
        if ev:
            role_map = {"support": "支持", "contrast": "对比", "counter": "反面"}
...
def format_sft_instance(sample_id, subtitle, ev_texts, fusion_partner_id, gpt_output, candidate_labels, current_mm_ref=None, partner_mm_ref=None, lang="zh", face_or_frame="face"):
    """
    Format SFT training instance for Agent-G.
...
def main():
    ...
    # 3. 读取 Agent-S 的 SFT 数据
    s_data = []
    if not os.path.exists(args.s_data_path):
        print(f"Error: Agent-S data not found at {args.s_data_path}")
        return
    ...
    print(f"Processing {len(s_data)} samples for Agent-G...")
    if not fusion_partner_map:
        raise RuntimeError(
            "Agent-G SFT construction now requires a precomputed fusion_partner_map from Phase 1c. "
    ...
    with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
        ...
        for item in s_data:
            ...
            ev_texts = {"support": None, "contrast": None, "counter": None}
            ...
            if any(ev_texts[role] is None for role in ["support", "contrast", "counter"]):
                continue
    ...
    output_path = os.path.join(args.output_dir, "agent_g_sft.jsonl")
    with open(output_path, "w", encoding="utf-8") as f:
        for item in g_sft_data:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")
            
    print(f"Agent-G SFT data saved to {output_path} ({len(g_sft_data)} samples)")
