...
S_SYSTEM_PROMPT_ZH = """你是一个多模态情感分析证据筛选器（Evidence Selector）。
...
S_SYSTEM_PROMPT_EN = """You are a multimodal emotion analysis Evidence Selector.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Construct SFT data for Agent-S")
    parser.add_argument("--dataset", type=str, default="mer2023", help="Target dataset (mer2023, meld, iemocap)")
    parser.add_argument("--q-data-path", type=str, default="retrieval/mmoa_lite/artifacts/sft_data/agent_q_sft.jsonl")
...
def format_sft_instance(sample_id, subtitle, candidates, selected_output, mm_ref=None, lang="zh", face_or_frame="face"):
    """
    Format SFT training instance for Agent-S.
    ...
    s_output_schema = '{"primary": {"id": "..."}, "confusion": {"id": "..."}, "counter": {"id": "..."}}'
    ...
    if lang == "zh":
        ...
        sft_input = (
            "System: You are an emotion analysis Evidence Selector. You will receive the current sample's "
...
def main():
    ...
    retriever = DoubleChannelRetriever(semantic_index_dir=args.index_dir)
    ...
    if not os.path.exists(args.q_data_path):
        print(f"Error: Agent-Q data not found at {args.q_data_path}")
        return
    ...
    print(f"Processing {len(q_data)} samples for Agent-S...")
    ...
    output_path = os.path.join(args.output_dir, "agent_s_sft.jsonl")
    with open(output_path, "w", encoding="utf-8") as f:
        for item in sft_data:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")
            
    print(f"Agent-S SFT data saved to {output_path} ({len(sft_data)} samples)")
