...
class MERCaptionPlus_Dataset(BaseDataset):
    def __init__(self, vis_processor=None, txt_processor=None, img_processor=None,
        ...
        primary_subtitle_columns = [col for col in ['chinese', 'zh', 'cn'] if col in df.columns]
        fallback_subtitle_columns = [col for col in ['english', 'en'] if col in df.columns]
        self.subtitle_lang = 'zh' if primary_subtitle_columns else 'en'
        for _, row in df.iterrows():
            name = row['name']
            subtitle = ""
            for column in primary_subtitle_columns + fallback_subtitle_columns:
                value = row[column]
                if pd.isna(value):
                    continue
