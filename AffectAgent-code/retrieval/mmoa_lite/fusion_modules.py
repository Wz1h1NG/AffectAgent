...
class SupportFusion(nn.Module):
    """
    SupportFusion (Pre-G 融合模块)
    利用门控交叉注意力机制 (Gated Cross-Attention)，将通道B检索到的感知最相似样本(fusion_partner)
    的多模态特征，融合到当前样本的多模态特征中，进行特征级增强。
    """
...
class ModalityMoE(nn.Module):
    """
    ModalityMoE (自适应模态平衡模块)
    对 SupportFusion 融合后的视听特征，利用混合专家网络 (MoE) 进行模态平衡。
    路由器根据全局视听特征决定激活哪些专家，专家网络分别负责不同模态主导的特征提炼。
    """
