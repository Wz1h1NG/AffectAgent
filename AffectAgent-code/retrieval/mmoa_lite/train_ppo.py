"""
train_ppo.py — AffectAgent MAPPO-style joint training entry point.
Load an AffectGPT or warm-start SFT checkpoint through Config + registry.
集成双通道检索、SupportFusion + ModalityMoE、V3 七维奖励系统。

核心流程（每个 step）：
  1. 从训练集采样一批样本，AffectGPT 提取多模态特征
  2. Rollout: Q → Channel A + Channel B → S → Fusion → G
  3. 计算 V3 奖励 (7 维分解 + credit assignment)
  4. 计算 policy gradient loss (带 KL 正则)
  5. 更新：LLM LoRA + SupportFusion + ModalityMoE
...
from retrieval.mmoa_lite.schemas import RolloutSample, RolloutResult, RewardBreakdown
...
class SequenceValueHead(nn.Module):
    ...
    def forward(self, hidden_state: torch.Tensor) -> torch.Tensor:
        ...
        # 对齐 dtype（LLM hidden states 可能为 fp16，value head 权重为 fp32）
        hidden_state = hidden_state.to(dtype=self.summary[0].weight.dtype)
        return self.summary(hidden_state).squeeze(-1)


class PolicyGradientUpdater:
    """
    PPO-style 策略梯度更新器。
    对齐 MMOA 使用的 TRL PPOTrainer 核心思想：
    - 使用 old_log_probs 构造 importance ratio
    - 使用 clipped surrogate objective
    - 使用 value baseline 估计 advantage
    - 保留 reference-model KL 正则
    """
    ...
    def _build_replay_buffer(
        ...
        reward_values = [float(item["reward"]) for item in trajectories]
        reward_mean = sum(reward_values) / len(reward_values) if reward_values else 0.0
        reward_std = max(
            (sum((r - reward_mean) ** 2 for r in reward_values) / len(reward_values)) ** 0.5,
            1e-8,
        ) if len(reward_values) > 1 else 1.0
        ...
        for trajectory in trajectories:
            ...
            with torch.no_grad():
                old_log_probs, old_state = orchestrator.compute_agent_log_probs_and_state(
                    result,
                    agent_name,
                    self.model.llama_model,
                )
                # 用 LoRA disable 技巧计算 reference log-probs（与 TRL PPOTrainer 一致）
                # 临时关闭 LoRA adapter，用同一份 base model 当 reference
                self.model.llama_model.disable_adapter_layers()
                ref_log_probs = orchestrator.compute_agent_log_probs(
                    result,
                    agent_name,
                    self.model.llama_model,
                )
                self.model.llama_model.enable_adapter_layers()
                old_value = self.value_head(old_state).squeeze() if old_state is not None else None

            if old_log_probs.numel() == 0 or old_state is None:
                continue
            return_value = torch.tensor(
                (reward_value - reward_mean) / reward_std,
                dtype=old_value.dtype,
                device=self.device,
            )
            advantage = return_value - old_value
        ...
        if len(replay_buffer) > 1:
            all_advantages = torch.stack([item["advantage"] for item in replay_buffer])
            adv_mean = all_advantages.mean()
            adv_std = all_advantages.std(unbiased=False).clamp_min(1e-8)
            for idx, item in enumerate(replay_buffer):
                item["advantage"] = ((all_advantages[idx] - adv_mean) / adv_std).detach()
    ...
    def update_step(
        ...
        """执行一次 PPO-style 更新（Q/S/G 序列按 credit assignment 已分配不同奖励）。"""
        if len(trajectories) == 0:
            return {"loss": 0.0, "mean_reward": 0.0, "mean_kl": 0.0, "policy_loss": 0.0, "value_loss": 0.0, "update_rounds": 0}

        # old_log_probs 必须在 eval 模式下计算，避免 dropout 随机性导致
        # 首轮 PPO epoch ratio ≠ 1.0
        self.model.llama_model.eval()
        self.value_head.eval()

        replay_buffer = self._build_replay_buffer(trajectories, orchestrator)
        ...
        total_kl = 0.0
        total_reward = sum(float(item["reward"]) for item in replay_buffer)
        count = 0
        update_rounds = 0
        ...
        for _ in range(self.ppo_epochs):
            # 使用梯度累积替代 loss 累积，避免同时保留所有计算图导致 OOM
            self.optimizer.zero_grad()
            epoch_count = 0
            epoch_loss_scalar = 0.0
            divisor = max(len(replay_buffer), 1)

            for item in replay_buffer:
                result = item["result"]
                agent_name = item["agent"]

                policy_log_probs, sequence_state = orchestrator.compute_agent_log_probs_and_state(
                    result,
                    agent_name,
                    self.model.llama_model,
                )
                if policy_log_probs.numel() == 0 or sequence_state is None:
                    continue

                log_ratio = torch.clamp(
                    policy_log_probs.sum() - item["old_log_probs"].sum(),
                    min=-20.0,
                    max=20.0,
                )
                ...
                value_pred = self.value_head(sequence_state).squeeze()
                old_value = item["old_value"]
                value_target = item["return"]
                ...
                kl = (policy_log_probs - item["ref_log_probs"]).mean()

                # 除以 divisor 使梯度累积等价于 batch mean
                sample_loss = (policy_loss + self.value_coef * value_loss + self.kl_coef * kl) / divisor
                sample_loss.backward()  # 每个 item 立即 backward 并释放计算图

                epoch_loss_scalar += float(sample_loss.detach()) * divisor
            ...
            self.optimizer.step()
            total_loss_value += epoch_loss_scalar / max(epoch_count, 1)
            update_rounds += 1
        ...
        return {
            "loss": total_loss_value / max(update_rounds, 1),
            "mean_reward": total_reward / max(len(replay_buffer), 1),
...
def parse_args():
    parser = argparse.ArgumentParser(description="MMOA-Lite V3 MAPPO Joint Training")
    ...
    parser.add_argument("--fusion-dim", type=int, default=0,
                        help="SupportFusion / MoE 的特征维度，<=0 时自动对齐 LLM hidden_size")
    parser.add_argument("--fusion-heads", type=int, default=8,
                        help="SupportFusion Cross-Attention 头数")
    ...
    parser.add_argument("--output-dir", type=str,
                        default="retrieval/mmoa_lite/artifacts/rl_checkpoints")
...
def main():
    ...
    # ──────────────── 1. 加载 AffectGPT 原始模型 ────────────────
    print("=" * 60)
    print("[1/6] Loading AffectGPT model (original weights, NOT SFT)...")
    ...
    # KL 正则不再需要加载第二份模型——使用 LoRA disable 技巧（与 TRL PPOTrainer 一致）
    # 计算 reference log-probs 时临时关闭 LoRA adapter，用同一份 base model 当 reference
    print("  Reference model: using LoRA disable trick (no extra model copy)")

    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  AffectGPT loaded. Trainable params (LoRA): {trainable_params:,}")

    # ──────────────── 2. 初始化 SupportFusion + MoE ────────────────
    print("=" * 60)
    print("[2/6] Initializing SupportFusion + ModalityMoE...")
    ...
    fusion_params = sum(p.numel() for p in support_fusion.parameters()) + \
                    sum(p.numel() for p in modality_moe.parameters())
    ...
    trainable_param_groups = [
        ...
        {"params": list(support_fusion.parameters()) + list(modality_moe.parameters()),
    ...
    # ──────────────── 6. 训练循环 ────────────────
    print("=" * 60)
    print("[6/6] Starting MMOA-Lite V3 MAPPO training...")
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_samples, batch_sample_data in pbar:
            ...
            rollout_results = orchestrator.batch_rollout(
            ...
            if global_step <= 5:
                for i, (rr, bd) in enumerate(zip(rollout_results, reward_breakdowns)):
                    ...
                    s_valid = rr.selector_output.valid if rr.selector_output else False
                    ...
                    print(f"  [DIAG step={global_step} sample={i}] "
                          f"Q={q_valid} S={s_valid} cand={n_cand} evi={n_evi} | "
                          f"gt={gt!r} | pred={g_pred!r} G={g_valid} | "
                          f"r_task={bd.r_task:.2f} p_hard={bd.p_hard:.2f} r_total={bd.r_total:.3f}")
                    if global_step <= 2:
                        g_raw = (g_out.raw_text[:200] if g_out else "None") if g_out else "None"
                        print(f"    G_raw: {g_raw}")

            # ──── 收集 PPO 序列 ────
            trajectories = orchestrator.collect_ppo_trajectories(rollout_results)

            # ──── 策略梯度更新 ────
            # update_step 内部会先 eval 算 old_log_probs，再 train 做 PPO
            # fusion 模块也需在 train 模式（orchestrator replay G 时会重跑 fusion）
            support_fusion.train()
            modality_moe.train()
            stats = updater.update_step(trajectories, orchestrator)
            ...
            batch_accuracy = sum(bd.r_task for bd in reward_breakdowns) / max(len(reward_breakdowns), 1)
            batch_r_total = sum(bd.r_total for bd in reward_breakdowns) / max(len(reward_breakdowns), 1)
            ...
            if global_step % args.save_steps == 0:
                ...
                # 保存 AffectGPT 可训练参数 (LoRA)
                trainable_state = get_trainable_state_dict(model)
                torch.save({"model": trainable_state}, os.path.join(ckpt_dir, "affectgpt_trainable.pth"))

                # 保存 Fusion 模块
                torch.save(support_fusion.state_dict(), os.path.join(ckpt_dir, "support_fusion.pth"))
                torch.save(modality_moe.state_dict(), os.path.join(ckpt_dir, "modality_moe.pth"))
                torch.save(value_head.state_dict(), os.path.join(ckpt_dir, "value_head.pth"))
                ...
                if current_avg_reward > best_reward:
                    ...
                    torch.save({"model": trainable_state}, os.path.join(best_dir, "affectgpt_trainable.pth"))
                    torch.save(support_fusion.state_dict(), os.path.join(best_dir, "support_fusion.pth"))
                    torch.save(modality_moe.state_dict(), os.path.join(best_dir, "modality_moe.pth"))
                    torch.save(value_head.state_dict(), os.path.join(best_dir, "value_head.pth"))
    ...
    trainable_state = get_trainable_state_dict(model)
    torch.save({"model": trainable_state}, os.path.join(final_dir, "affectgpt_trainable.pth"))
    torch.save(support_fusion.state_dict(), os.path.join(final_dir, "support_fusion.pth"))
    torch.save(modality_moe.state_dict(), os.path.join(final_dir, "modality_moe.pth"))
    torch.save(value_head.state_dict(), os.path.join(final_dir, "value_head.pth"))
